...
@testset "LanczosSincFilter" begin
    l = RayCaster.LanczosSincFilter(Point2f(4f0), 3f0)
...
@testset "Film testing" begin
    filter = RayCaster.LanczosSincFilter(Point2f(4f0), 3f0)
    film = RayCaster.Film(
        Point2f(1920f0, 1080f0), RayCaster.Bounds2(Point2f(0f0), Point2f(1f0)),
        filter, 35f0, 1f0, "output.png",
    )
    @test size(film.pixels) == (1080, 1920)
    @test RayCaster.get_sample_bounds(film) == RayCaster.Bounds2(Point2f(-3f0), Point2f(1924f0, 1084f0))
end
...
@testset "FilmTile testing" begin
    ...
    tile = RayCaster.FilmTile(film, RayCaster.Bounds2(Point2f(1f0), Point2f(10f0)))
    @test size(tile.pixels) == (14, 14)
    @test tile.bounds == RayCaster.Bounds2(Point2f(1f0), Point2f(14f0))
    # Pixels in 1:5 radius should be affected.
    for i in 1:5
        @test tile.pixels[i, i].filter_weight_sum ≈ 0f0
    end
    RayCaster.add_sample!(tile, Point2f(1f0), RayCaster.RGBSpectrum(1f0))
    ...
    RayCaster.merge_film_tile!(film, tile)
    for (i, j) in zip(1:4, 2:5)
    ...
    # Test shifted tile.
    tile = RayCaster.FilmTile(film, RayCaster.Bounds2(Point2f(10f0), Point2f(60f0)))
    @test size(tile.pixels) == (59, 59) # Account for filter radius shift.
    @test tile.bounds == RayCaster.Bounds2(Point2f(6f0), Point2f(64f0))
    # Add to [20, 20] pixel
    ...
    RayCaster.add_sample!(tile, Point2f(20f0), RayCaster.RGBSpectrum(1f0))
    # Symmetrical.
...
@testset "Perspective Camera" begin
    ...
    camera = RayCaster.PerspectiveCamera(
        RayCaster.translate(Vec3f(0)), RayCaster.Bounds2(Point2f(0), Point2f(10)),
        0f0, 1f0, 0f0, 700f0, 45f0, film,
    )

    sample1 = RayCaster.CameraSample(Point2f(1f0), Point2f(1f0), 0f0)
    ray1, contribution = RayCaster.generate_ray(camera, sample1)
    sample2 = RayCaster.CameraSample(
        Point2f(film.resolution[1]), Point2f(film.resolution[2]), 0f0,
    )
    ray2, contribution = RayCaster.generate_ray(camera, sample2)
    ...
    ray_differential, contribution = RayCaster.generate_ray_differential(
        camera, sample1,
    )
