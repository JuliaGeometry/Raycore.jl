...
@kernel function raytrace_kernel_v1!(
    ...
    if x <= width && y <= height
        ...
        # Using ntuple for compile-time unrolling
        samples = ntuple(_ -> sample_light(bvh, ctx, width, height, camera_pos, focal_length, aspect, x, y, sky_color), NSamples)
        color = sum(samples)
        @inbounds img[y, x] = to_rgb(color ./ Float32(NSamples))
    end
end
```
...
img_v1 = trace_gpu(raytrace_kernel_v1!, img_gpu, bvh_gpu, ctx_gpu, samples_per_pixel=8)
...
```julia (editor=true, logging=false, output=true)
# Benchmark kernel v1 for later comparison
bench_img = GArray(fill(RGBf(0, 0, 0), 256, 256))
bench_kernel_v1 = @benchmark trace_gpu($raytrace_kernel_v1!, $bench_img, $bvh_gpu, $ctx_gpu, samples_per_pixel=4)
```
...
@kernel function raytrace_kernel_unrolled!(
    ...
    if x <= width && y <= height
        ...
        samples = ntuple(_ -> sample_light(bvh, ctx, width, height, camera_pos,
                                           focal_length, aspect, x, y, sky_color), NSamples)
        color = sum(samples)
        @inbounds img[y, x] = to_rgb(color ./ Float32(NSamples))
    end
end

img_unrolled = trace_gpu(raytrace_kernel_unrolled!, img_gpu, bvh_gpu, ctx_gpu, samples_per_pixel=4)
Array(img_unrolled)
```
```julia (editor=true, logging=false, output=true)
# Benchmark kernel unrolled for later comparison
bench_kernel_unrolled = @benchmark trace_gpu($raytrace_kernel_unrolled!, $bench_img, $bvh_gpu, $ctx_gpu, samples_per_pixel=4)
```
...
```
```julia (editor=true, logging=false, output=true)
# Benchmark tiled kernel (32, 16) for later comparison
bench_kernel_tiled_32_16 = @benchmark trace_gpu($raytrace_kernel_tiled!, $bench_img, $bvh_gpu, $ctx_gpu;
...
```julia (editor=true, logging=false, output=true)
# Benchmark the tiled kernel with (32, 32) tiles for comparison
bench_kernel_tiled_32_32 = @benchmark trace_gpu($raytrace_kernel_tiled!, $bench_img, $bvh_gpu, $ctx_gpu;
...
benchmarks = [bench_kernel_v1, bench_kernel_unrolled, bench_kernel_tiled_32_16, bench_kernel_tiled_32_32]
labels = ["Baseline\n(loop)", "Unrolled\n(4 samples)", "Tiled\n(32×16)", "Tiled\n(32×32)"]
